...
public class BtnEsc extends GuiButton
{

    public String m_BtnName = "";
    public float m_Alpha = 0f;
    public BtnEsc(int buttonId, int x, int y, String buttonText, String btnName)
    {
        ...
        float currutionWidth = 235f / 3f;
        float currutionHeight = 68f / 3f;
...
//    }
    public BtnEsc(int buttonId, int x, int y, String buttonText) {
        super(buttonId, x, y, buttonText);
    }
    ...
    public void drawButton(Minecraft mc, int mouseX, int mouseY, float partialTicks)
    {
        ...
        if(!m_BtnName.equals("뒤로가기"))
        {
            ...
            {
                mc.renderEngine.bindTexture(new ResourceLocation(GroundWorld.RESOURCEID, "textures\\gui\\esc\\btn\\"+m_BtnName+"_select.png"));
                drawTexture(this.x, this.y, currutionWidth, currutionHeight, 0, 0, 1, 1, 3, 1f);

                mc.renderEngine.bindTexture(new ResourceLocation(GroundWorld.RESOURCEID, "textures\\gui\\esc\\btn\\select.png"));
                drawTexture(this.x - 6, this.y + 5, 42f/3f, 42f/3f, 0, 0, 1, 1, 3, 1f);
            }
            else
            {
                mc.renderEngine.bindTexture(new ResourceLocation(GroundWorld.RESOURCEID, "textures\\gui\\esc\\btn\\"+m_BtnName+".png"));
                drawTexture(this.x, this.y, currutionWidth, currutionHeight, 0, 0, 1, 1, 3, 1f);
            }
        ...
        else
        {
            if(active.equals("_on"))
            {
                GlStateManager.pushMatrix();
                {
                    ...
                    mc.renderEngine.bindTexture(new ResourceLocation(GroundWorld.RESOURCEID, "textures\\gui\\phone\\call\\btn\\"+m_BtnName+".png"));
