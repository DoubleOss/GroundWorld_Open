...
public class SPacketCallNot implements IMessage, IMessageHandler<SPacketCallNot, SPacketCallNot>
{
    ...
    public SPacketCallNot(){}

    public SPacketCallNot(Variable.WATCH_STATUS status, String senderPlayer)
    {
    ...
    @Override
    public SPacketCallNot onMessage(SPacketCallNot message, MessageContext ctx)
    {
        EntityPlayerMP player = ctx.getServerHandler().player.getServer().getPlayerList().getPlayerByUsername(message.m_senderPlayer);
        player.sendMessage(new TextComponentString(" 지금 상대방이 전화를 받을 수 없는 상태입니다."));
        if(player != null)
        {
