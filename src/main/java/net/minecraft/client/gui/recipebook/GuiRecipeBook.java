...
public class GuiRecipeBook extends Gui implements IRecipeUpdateListener
{
    ...
    {
        this.xOffset = p_193014_1_ ? 0 : 200;
        ...
        this.searchBar = new GuiTextField(0, this.mc.fontRenderer, i + 127, j - 6, 80, this.mc.fontRenderer.FONT_HEIGHT + 5);
        this.searchBar.setMaxStringLength(50);
        this.searchBar.setEnableBackgroundDrawing(false);
        this.searchBar.setVisible(true);
        this.searchBar.setTextColor(16777215);
        this.recipeBookPage.init(this.mc, i+102, j);
    ...
    public void render(int mouseX, int mouseY, float partialTicks)
    {
        if (this.isVisible())
        {
            ...
            float searchWidth = 298f/3f;
            float searchHeight = 50f/3f;

            float search2Width = 36f/3f;
            float search2Height = 36f/3f;
            ...
            GlStateManager.translate(0.0F, 0.0F, 1f);
            mc.renderEngine.bindTexture(new ResourceLocation(GroundWorld.RESOURCEID, "textures\\hud\\hud\\inventory_guide(search).png"));
            Render.drawTexture(scaleWidth/2 - searchWidth/2 - 118, scaleHeight/2 - searchHeight/2 - 85, searchWidth, searchHeight, 0, 0, 1, 1, 0, 1);
    ...
    {
        this.ghostRecipe.render(this.mc, p_191864_1_, p_191864_2_, false, p_191864_4_);
    }
        ...
        if (this.isVisible() && !this.mc.player.isSpectator())
        {
            if (this.recipeBookPage.mouseClicked(p_191862_1_, p_191862_2_, p_191862_3_, (this.width - 147) / 2 - this.xOffset, (this.height - 166) / 2, 147, 166))
            {
                ...
                if (irecipe != null && recipelist != null)
                {
                    ...
                    System.out.println(irecipe.getRecipeOutput() + " " + recipelist.isCraftable(irecipe));
                    System.out.println(this.ghostRecipe.getRecipe() == irecipe);

//                    //띵숲 코드 가져와서 아이템 있는지 없는지 비교해서 허용시키기
